                ...
                <ResponsiveContainer width="100%" height={140}>
                  <AreaChart data={stats.daily} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                    ...
                    <Tooltip
                      ...
                      formatter={(v) => [fmtFull(v as number), 'Выручка']}
                ...
                <ResponsiveContainer width="100%" height={100}>
                  <BarChart data={stats.peak_hours} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                    ...
                    <Tooltip
                      ...
                      labelStyle={{ color: 'rgba(255,255,255,0.5)', fontSize: 11 }}
                      itemStyle={{ color: '#fff', fontSize: 12 }}
                      formatter={(v) => [v as number, 'Записей']}
                  ...
                  return (
                    <div key={s.id} className="p-4 rounded-2xl bg-white/5">
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-bold text-white text-sm">Лаборатория</span>
                        <span className="text-xs text-white/40">{s.count} записей</span>
                      </div>
                      <div className="h-1.5 bg-white/10 rounded-full overflow-hidden mb-2">
                        <div className="h-full bg-white/60 rounded-full transition-all" style={{ width: `${pct}%` }} />
                      </div>
                      <div className="text-sm font-semibold text-white">{fmtFull(s.revenue)}</div>
                    </div>
                  )
                })}
              </div>
                ...
                <div>
                  ...
                  {stats && (
                    <div className="text-xs text-green-400 mt-1">
                      ≈ {(stats.revenue.month * Number(emp.revenue_percent) / 100).toLocaleString('ru-RU')} ₽ за месяц
                    </div>
                  )}
                </div>
          ...
          <div className="p-4 rounded-2xl bg-white/5 mt-2">
            ...
            <div className="space-y-3">
              {employees.map(emp => {
                ...
                const empShare = monthRev * (Number(emp.revenue_percent) / 100)
                return (
