...
import { Mic2, Lock, X, Users, UserPlus, Check, ChevronLeft, ChevronRight,
         Calendar, BarChart2, Ban, PlusCircle } from 'lucide-react'
import {
...
import type { Lead, Client, DayData, DaySlot, CalendarDay } from '../api'
import { SERVICES } from '../data'
import { useAppContext } from '../App'
import { OwnerDashboard } from './OwnerDashboard'

type View = 'dashboard' | 'calendar' | 'day' | 'bookings' | 'clients' | 'pin' | 'owner'
    ...
    <div className="pb-nav bg-[#0E0E0E] min-h-screen relative">
      ...
      {sheet && (
        <>
          ...
          />
          <div className="fixed left-0 right-0 bottom-0 z-50 bg-[#111] border-t border-[#2A2A2A] rounded-t-3xl animate-slide-up"
...
function QuickBookForm({ date, time, onClose, onCreated }: {
  ...
  const inp = 'w-full px-3 py-2.5 rounded-xl bg-white/8 text-white text-sm placeholder-white/20 outline-none focus:ring-1 focus:ring-[#C17BFF]/40 border border-[#2A2A2A]'
...
function BlockSlotForm({ date, time, onClose, onBlocked }: {
  ...
  const inp = 'w-full px-3 py-2.5 rounded-xl bg-white/8 text-white text-sm placeholder-white/20 outline-none focus:ring-1 focus:ring-[#FF4B4B]/40 border border-[#2A2A2A]'
