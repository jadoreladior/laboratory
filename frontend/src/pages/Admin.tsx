...
import { getAdminBookings, confirmBooking, cancelBooking, verifyOwnerPin, getClients, addClient } from '../api'
import type { Lead, Client } from '../api'
import { STUDIOS } from '../data'
import { useAppContext } from '../App'
import { Mic2, Lock, X, Users, UserPlus, Check } from 'lucide-react'
import { OwnerDashboard } from './OwnerDashboard'

type View = 'dashboard' | 'bookings' | 'clients' | 'owner' | 'pin'
type FilterStatus = 'all' | 'pending' | 'confirmed' | 'cancelled'

type RawBooking = Lead
...
export function Admin() {
  ...
  const load = () => {
    setLoading(true)
    Promise.all([getAdminBookings(), getClients()])
      .then(([b, c]) => { setBookings(b); setClients(c) })
      .catch(() => {})
  ...
  if (view === 'clients') {
    return <ClientsView clients={clients} setClients={setClients} onBack={() => setView('dashboard')} />
  }
    ...
    return (
      <div className="pb-nav flex flex-col" style={{ minHeight: '100svh', background: 'linear-gradient(160deg, #111111 0%, #0a0a0a 50%, #0d0d0d 100%)' }}>
        ...
        <div className="flex flex-col items-center" style={{ flex: 1, justifyContent: 'center', gap: 0, position: 'relative', zIndex: 1, paddingBottom: 24 }}>
          ...
          <h2 style={{ color: 'white', fontSize: 20, fontWeight: 900, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 4 }}>Лаборатория</h2>
              ...
              return (
                <div key={b.id} className="p-4 rounded-2xl bg-white/5">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="font-semibold text-white text-sm">{b.service}</div>
                      <div className="text-xs text-white/50 mt-0.5 flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full inline-block flex-shrink-0 bg-white/40" />
                        {b.client_name || 'Лаборатория'}
                  ...
                  <div className="text-xs text-white/30 mb-3">
                    Предоплата: <span className="text-white/60 font-medium">{Number(b.prepay_amount).toLocaleString()} ₽</span>
                  </div>
          ...
          {upcoming ? (
            <div className="p-3 rounded-2xl bg-white/5 flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl flex-shrink-0 overflow-hidden">
                <img src={STUDIOS[0].images[0]} alt="" className="w-full h-full object-cover" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-white truncate">{upcoming.service}</div>
                <div className="text-xs text-white/40 mt-0.5">
                  {upcoming.booking_date} · {upcoming.booking_time} · {upcoming.client_name}
                </div>
              </div>
  ...
  return (
    <div className="pb-nav animate-fade-in">
      <div className="px-4 pt-6 pb-5">
        ...
        <p className="text-sm dark:text-white/50 text-gray-500 mt-1">Лаборатория</p>
      </div>
      ...
      <div className="px-4 grid grid-cols-2 gap-3 stagger">
        ...
        <button
          ...
          className="relative p-5 rounded-3xl dark:bg-white/5 bg-black/5 text-left active:scale-95 transition-transform"
        >
          <div className="dark:text-white/70 text-gray-600 mb-3"><Users size={22} strokeWidth={1.5} /></div>
          <div className="font-bold dark:text-white text-gray-900 text-sm">Клиенты</div>
          <div className="text-xs dark:text-white/40 text-gray-400 mt-0.5">{clients.length} в базе</div>
        </button>

        <button
          onClick={() => setView('pin')}
          className="relative p-5 rounded-3xl bg-white/5 border border-white/10 text-left active:scale-95 transition-transform col-span-2"
        >
...
function ClientsView({
  clients, setClients, onBack,
}: { clients: Client[]; setClients: React.Dispatch<React.SetStateAction<Client[]>>; onBack: () => void }) {
  ...
  return (
    <div className="pb-nav animate-fade-in flex flex-col" style={{ minHeight: '100dvh' }}>
      <div className="px-4 pt-6 pb-4 flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-white flex-1">Клиенты</h1>
        <button
          onClick={() => setShowForm(v => !v)}
          className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-white/60 active:scale-95"
        >
          <UserPlus size={15} />
        </button>
      </div>
      ...
      {showForm && (
        <div className="px-4 mb-4 space-y-2">
          <div className="p-4 rounded-2xl bg-white/5 space-y-2">
            <input className={inp} placeholder="Имя *" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} />
            <input className={inp} placeholder="Телефон" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} />
            <input className={inp} placeholder="@username" value={form.username} onChange={e => setForm(p => ({ ...p, username: e.target.value }))} />
            <input className={inp} placeholder="Заметки" value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} />
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={() => setShowForm(false)} className="w-10 rounded-xl bg-white/10 flex items-center justify-center text-white/60">
                <X size={16} />
              </button>
            </div>
          </div>
      ...
      <div className="px-4 space-y-2 flex-1">
        {clients.length === 0 ? (
          <div className="text-center py-12 text-white/30 text-sm">Клиентов пока нет</div>
        ) : clients.map(c => (
          <div key={c.id} className="p-4 rounded-2xl bg-white/5">
            <div className="font-semibold text-white text-sm">{c.name}</div>
            <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1">
              {c.phone && <span className="text-xs text-white/50">{c.phone}</span>}
              {c.username && <span className="text-xs text-white/40">@{c.username}</span>}
              {c.telegram_id && <span className="text-xs text-white/30">TG: {c.telegram_id}</span>}
            </div>
            {c.notes && <div className="text-xs text-white/30 mt-1 italic">{c.notes}</div>}
          </div>
        ))}
      </div>
    </div>
