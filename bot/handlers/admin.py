...
async def admin_panel(message: Message):
    ...
    async with httpx.AsyncClient() as client:
        r = await client.get(f"{BACKEND_URL}/api/stats")
    stats = r.json() if r.status_code == 200 else {}

    text = (
        f"🛠 <b>CRM · Лаборатория</b>\n\n"
        f"👥 Клиентов: <b>{stats.get('total_clients', '—')}</b>\n"
        f"📋 Заявок: <b>{stats.get('statuses', {}).get('pending', '—')}</b> ожидают\n"
        f"💰 Выручка (мес): <b>{stats.get('revenue', {}).get('month', '—')}</b> ₽\n\n"
        f"<b>Команды:</b>\n"
...
async def today_bookings(message: Message):
    ...
    async with httpx.AsyncClient() as client:
        r = await client.get(f"{BACKEND_URL}/api/leads?status=")

    if r.status_code != 200:
        await message.answer("Ошибка загрузки заявок.")
        return

    all_leads = r.json()
    bookings = [b for b in all_leads if b.get("booking_date") == today]

    if not bookings:
        await message.answer(f"📅 На <b>{today}</b> заявок нет.", parse_mode="HTML")
        return

    lines = [f"📅 <b>Заявки на {today}:</b>\n"]
    for b in bookings:
        status_icon = {"pending": "⏳", "confirmed": "✅", "completed": "🏁", "cancelled": "❌"}.get(b.get("status"), "•")
        price = int(float(b.get("total_price", 0) or 0))
        lines.append(
            f"{status_icon} <b>{b.get('booking_time')}</b>\n"
            f"   {b.get('service')} · {price:,} ₽\n"
            f"   {b.get('client_name')} · ID #{b.get('id')}\n"
        )
