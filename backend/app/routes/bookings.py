...
from ..notifier import notify_booking_created, notify_booking_cancelled, notify_booking_confirmed
import asyncio
...
async def confirm_booking(booking_id: int, db: AsyncSession = Depends(get_db)):
    ...
    user = await db.get(User, booking.user_id)
    asyncio.create_task(notify_booking_confirmed(booking, user))
